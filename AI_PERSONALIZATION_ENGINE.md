# AI Personalization Engine — Technical Documentation
...
The Personalization Engine sits **between lead ingestion and email sending**. Its job is to take raw lead data (an email address + company name) and produce a fully personalized email that feels hand-crafted — not templated.

**Core Pipeline:**

```
Lead Added → Enrichment → AI Generation → Template Rendering → Email Sent → Tracking → Follow-up
```

Everything runs inside `app.py` (Python/FastAPI). There is **no separate microservice** — the AI calls go directly to the Hugging Face free Inference API over HTTPS.
...
│                                                                 │
│  ┌──────────────┐    ┌───────────────────┐    ┌──────────────┐  │
│  │ Lead Ingestion│───▸│ Lead Enrichment   │───▸│ MongoDB Save │  │
│  │ (POST /recr.) │    │ extract_first_name│    │              │  │
│  │               │    │ normalize_company │    │              │  │
│  └──────────────┘    └───────────────────┘    └──────────────┘  │
│                                                                 │
│  ┌──────────────┐    ┌───────────────────┐    ┌──────────────┐  │
│  │ Send Email   │───▸│ AI Personalization│───▸│ build_email  │  │
│  │ (POST /send) │    │ _hf_generate()    │    │ HTML render  │  │
│  │              │    │ company_sentence   │    │              │  │
│  │              │    │ opening_line       │    │              │  │
│  │              │    │ subject_lines      │    │              │  │
│  └──────────────┘    └───────────────────┘    └──────────────┘  │
...
│           ▼                                                     │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ Tracking: open pixel + click redirect + reply checker    │   │
│  │ Engagement: Cold → Warm → Hot                            │   │
│  │ Follow-up: Behavioral branching based on signals         │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
### 3.1 First Name Extraction — `extract_first_name(email)`
...
| Step | Action | Example |
|------|--------|---------|
| 1 | Validate: check for `@` symbol | `john.smith@stripe.com` → valid |
...
If the email is `alexandersmith@company.com`, there's no separator to split on. Taking the first 5 characters (`Alexa`) gives a reasonable approximation. The user specified "4–6 characters" — 5 is the middle ground.

### 3.2 Company Normalization — `normalize_company(name)`
...
Enrichment is applied in **three places** — every entry point for leads:

1. **`POST /recruiters`** — single recruiter add
2. **`POST /recruiters/import-csv`** — file upload
3. **`POST /recruiters/import-text`** — pasted CSV text
...
- **No separate server** — just an HTTPS API call
- **Good models** — `Qwen/Qwen2.5-Coder-32B-Instruct` is a 32-billion parameter model
...
- **`HF_API_KEY`** — your free Hugging Face token (get it from https://huggingface.co/settings/tokens)
- **`HF_MODEL`** — the model to use (Qwen 32B is free and capable)
- **`HF_API_URL`** — the OpenAI-compatible chat/completions endpoint hosted by Hugging Face

### 4.3 The Core Generation Function — `_hf_generate(prompt, max_tokens)`
...
**Error Handling:**
- If `HF_API_KEY` is empty → returns `""` (skips AI, email still sends)
- If API returns non-200 → logs warning, returns `""` (graceful degradation)
- If request times out → logs error, returns `""` (email sends without AI content)
...
### 5.1 Company Sentence — `generate_company_sentence(company)`
...
- `"under 20 words"` — constrains length
- `"engineering-focused"` — matches the candidate profile
- `"Avoid generic phrases"` — prevents bland output
- `"Output only the sentence"` — prevents preamble like "Here's a sentence..."
...
### 5.2 Opening Line — `generate_opening_line(company)`
...
- `"Natural, professional tone"` — avoids marketing speak
- `"max 20 words"` — keeps it punchy
...
### 5.3 Subject Lines — `generate_subject_lines(company)`
...
| 2nd | `1 % 5 = 1` | `Backend Intern — Google` |
...
**Open Tracking** — invisible 1×1 pixel image:
```html
<img src="https://yourserver.com/track/open/john@stripe.com" width="1" height="1" />
```
When the email client loads images, it requests this URL → the server marks `opened: true`.
...
**Reply Tracking** — IMAP inbox check:
...
query = {
    ...
    "replied": False,
    "$or": [
        {   # HOT: Resume clicked but no reply → follow up in 36 hours
            "clicked": True,
            "followupStage": {"$in": [0, None]},
            "sentAt": {"$lte": now - timedelta(hours=36)}
        },
        {   # WARM: Opened but no click → follow up in 3 days
            "opened": True, "clicked": False,
            "followupStage": {"$in": [0, None]},
            "sentAt": {"$lte": now - timedelta(days=3)}
        },
        {   # COLD: Not opened → resend with new subject in 4 days
            "opened": False,
            "followupStage": {"$in": [0, None]},
...
|----------|---------|---------|------|
| **Hot** (clicked) | 36 hours | `Glad you saw my resume \| Kavya @ {company}` | Direct — "I noticed you viewed my resume" |
| **Warm** (opened) | 3 days | `Quick question about {company} internship` | Light — "Just checking in..." |
| **Cold** (not opened) | 4 days | `Quick question about internships` | Resend with **new subject line** |
| **Breakup** (stage 2) | 6 days after follow-up | `Wrapping up \| Summer '26 Intern @{company}` | Final — "I won't reach out again" |

### Why Different Timing?

- **Clicked (Hot):** They showed strong interest by viewing your resume. Strike while the iron is hot — 24-48 hours (we use 36 as the midpoint).
...
- Without caching: 10 API calls × ~2 seconds = 20 seconds wasted.
...
- Fast — Python dict lookup is O(1).
- Good enough — the cache persists as long as the server runs. If the server restarts, sentences are regenerated (which is fine, costs nothing on the free tier).
...
### Step 2 — AI Generation (3 API calls)
```
...
• 10k Daily Traffic: Led a team of 5 to build a high-traffic system.
• Production AI: Developed RAG systems and CMS Microservices.
• LeetCode Knight (1800+) | CF Expert (1700+)
...
### Step 6 — Follow-up Scenarios
```
Day 1: Email sent
Day 2: John opens email → status: Warm
Day 4: No reply → Follow-up sent ("Just checking in...")
Day 10: No reply → Breakup email sent ("I won't reach out again")
```
